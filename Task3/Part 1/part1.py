...
if __name__ == "__main__":
    ...
    new_joined_table = rideshare_data.join(taxi_zone_lookup_df, rideshare_data.pickup_location == taxi_zone_lookup_df.LocationID, 'inner') # Join operation is applied

    new_joined_table = new_joined_table.withColumnRenamed('Borough', 'Pickup_Borough') # New column name as required
    
    final_table = new_joined_table.groupBy('Pickup_Borough', 'Month').count() # Grouping by 'Pickup_Borough' and 'Month' and applying count function to get total number of pickups from that borough each month
