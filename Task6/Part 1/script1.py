...
if __name__ == "__main__":
    ...
    df = joined_table.groupBy('Pickup_Borough', 'time_of_day').count() # Grouping data by the Pickup Boroughs and time of the day to find the trip count at each time of the day for every borough.

    df = df.filter((col("count") > 0) & (col("count") < 1000)) # Filtering the data to only display boroughs with trip count greater than 0 and less than 1000
